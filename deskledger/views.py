...
from bookkeeping.models import Income, Expense, RecurringEntry
...
def dashboard(request):
    """
    Dashboard view for DeskLedger Local Edition.
    ...
    subscription_monthly = (
        RecurringEntry.objects.filter(
            user=user, is_subscription=True, is_active=True
        )
        .aggregate(total=Sum("amount"))
        .get("total")
        or 0
    )
    subscription_count = RecurringEntry.objects.filter(
        user=user, is_subscription=True, is_active=True
    ).count()

    # -------------------------------
    # MONTHLY CHART DATA (12 months for selected tax year)
    # -------------------------------
    income_by_month = (
        Income.objects.filter(user=user, date__gte=tax_year_start, date__lte=tax_year_end)
    ...
    expense_by_month = (
        Expense.objects.filter(user=user, date__gte=tax_year_start, date__lte=tax_year_end)
        .annotate(month=TruncMonth("date"))
        .values("month")
        .annotate(total=Sum("amount"))
        .order_by("month")
    ...
    # Build aligned month labels and series
    income_map = {r["month"].strftime("%b %Y"): float(r["total"]) for r in income_by_month}
    expense_map = {r["month"].strftime("%b %Y"): float(r["total"]) for r in expense_by_month}
    chart_labels = sorted(set(list(income_map.keys()) + list(expense_map.keys())))
    chart_income = [income_map.get(m, 0) for m in chart_labels]
    chart_expenses = [expense_map.get(m, 0) for m in chart_labels]
