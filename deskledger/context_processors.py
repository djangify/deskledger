...
def version(request):
    ...
    return {"DESKLEDGER_VERSION": settings.DESKLEDGER_VERSION}


def currency(request):
    """
    Make the logged-in user's currency symbol/code available in every template.
    Defaults to £ (GBP) for anonymous users.
    """
    symbol = "£"
    code = "GBP"
    if hasattr(request, "user") and request.user.is_authenticated:
        symbol = getattr(request.user, "currency_symbol", "£")
        code = getattr(request.user, "currency", "GBP") or "GBP"
    return {"currency_symbol": symbol, "currency_code": code}
