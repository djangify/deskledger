...
class BookkeepingConfig(AppConfig):
    ...
    def get_database_path(self):
        ...
        which works correctly for both the packaged .exe and local runs.
        """
    ...
    @staticmethod
    def _is_serving():
        """True only when this process is running the web server (dev server,
        gunicorn/waitress/uvicorn/daphne, or the desktop app) — not a one-off
        management command or a PyInstaller build."""
        argv = sys.argv or []
        if argv and "pyinstaller" in argv[0].lower():
            return False
        ...
        return any(s in prog for s in ("gunicorn", "waitress", "uvicorn", "daphne"))
    ...
    def run_daily_backup(self):
        ...
        try:
            ...
            if not DB_PATH.exists():
                print(f"No database file found at {DB_PATH}. Backup skipped.")
                return
