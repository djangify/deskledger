...
from bookkeeping.ocr import extract_receipt_data
...
def expense_list(request):
    ...
    # FIX: Only filter by tax year if one is selected AND it's not "all"
    ...
    # FIX: Get quarters from the FILTERED queryset
    ...
    return render(
        ...
        "bookkeeping/expense/expense_list.html",
        {
            ...
            "quarters": quarters,
            # FIX: Pass selected_tax_year to template
...
def ocr_receipt(request):
    """
    AJAX endpoint. Accepts a receipt file upload, runs OCR using the
    user's own API key, and returns extracted fields as JSON.
    POST body: multipart with 'receipt' file field.
    """
    user = request.user
    if not user.has_ocr_configured():
        return JsonResponse(
            {
                "error": "OCR not configured. Go to Account Settings and add your API key.",
                "setup_url": "/accounts/settings/",
            },
            status=400,
        )
    ...
    result = extract_receipt_data(
        file_field=receipt_file,
        api_key=user.ocr_api_key,
